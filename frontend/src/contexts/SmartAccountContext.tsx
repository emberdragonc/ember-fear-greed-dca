// SmartAccountContext.tsx - Shared smart account state across components
'use client';
...
export function SmartAccountProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (isConnected && walletClient && state.status === 'idle') {
      loadExistingAccount();
    }
  }, [isConnected, walletClient, state.status, loadExistingAccount]);
  ...
  const value: SmartAccountContextValue = {
    state,
    createSmartAccount,
    loadExistingAccount,
    clearStoredAddress,
...
export function useSmartAccountContext() {
  ...
  if (!context) {
    throw new Error('useSmartAccountContext must be used within SmartAccountProvider');
  }
  return context;
}
