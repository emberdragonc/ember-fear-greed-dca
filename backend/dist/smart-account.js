...
const BASE_CONTRACTS_V1_3 = DELEGATOR_CONTRACTS['1.3.0']?.['8453'];
const SIMPLE_FACTORY = (BASE_CONTRACTS_V1_3?.SimpleFactory || '0x69Aa2f9fe1572F1B640E1bbc512f5c3a734fc77c');
const BASE_IMPLEMENTATIONS = {
    HybridDeleGatorImpl: (BASE_CONTRACTS_V1_3?.HybridDeleGatorImpl || '0x48dBe696A4D990079e039489bA2053B36E8FFEC4'),
    MultiSigDeleGatorImpl: (BASE_CONTRACTS_V1_3?.MultiSigDeleGatorImpl || '0x0000000000000000000000000000000000000000'),
};
// Cached backend smart account (initialized on first use)
let _backendSmartAccount = null;
...
export async function ensureUserSmartAccountDeployed(smartAccountAddress, userEOA) {
    try {
...
export async function deployUndeployedAccounts(delegations) {
    ...
    for (const delegation of undeployed) {
        const smartAccountAddress = delegation.smart_account_address;
        const userEOA = delegation.user_address;
