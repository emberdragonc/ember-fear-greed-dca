...
const backendAccount = privateKeyToAccount(process.env.BACKEND_PRIVATE_KEY);
const walletClient = createWalletClient({
    account: backendAccount,
    chain: base,
    transport: http('https://base.drpc.org'),
});
async function getSwapData(amount, swapper) {
    // Step 1: Get quote
...
async function main() {
    ...
    const methodEnforcerTerms = signedDelegation.caveats?.find((c) => c.enforcer.toLowerCase() === '0x2c21fd0cb9dc8445cb3fb0dc5e7bb0aca01842b5')?.terms;
    ...
    if (methodEnforcerTerms) {
        ...
        console.log('Allowed method IDs:', methodsAllowed.map((m) => '0x' + m));
