...
import { FG_THRESHOLDS, sleep, } from './config';
...
// ============ FEAR & GREED ============
async function fetchFearGreedInternal() {
...
async function fetchFearGreed() {
    const { result, error, attempts } = await withRetry(fetchFearGreedInternal, { operation: 'fetchFearGreed' });
    if (!result) {
...
export function calculateDecision(fgValue) {
...
async function runDCA() {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (walletData) {
            await logExecution(walletData.delegation.id, walletData.delegation.user_address, fg?.value ?? 50, decision, result);
            if (result.success) {
                successCount++;
