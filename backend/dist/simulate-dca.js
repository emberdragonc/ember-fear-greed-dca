 ...
 */
import { createPublicClient, http, formatUnits, encodeFunctionData, erc20Abi } from 'viem';
...
async function simulateDCA() {
    ...
    for (const d of delegations.slice(0, 5)) { // Limit to 5 for simulation
        const smartAccount = d.smart_account_address;
        // Get balances
    ...
    for (const r of results) {
        ...
        if (r.details.changes)
            console.log(`  Asset changes: ${r.details.changes}`);
        console.log('');
        if (r.simulation.includes('PASS'))
            passed++;
        else if (r.simulation.includes('FAIL'))
            failed++;
        else
            skipped++;
    }
