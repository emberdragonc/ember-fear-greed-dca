...
import { CHAIN_ID, TRADING_API, ADDRESSES, ALCHEMY_RPC, SLIPPAGE_LARGE_BPS, MIN_DELEGATION_VALUE_USD, MIN_WALLET_VALUE_USD, QUOTE_VALIDITY_MS, MAX_QUOTES_PER_CYCLE, OPTIMAL_BATCH_SIZE, BATCH_DELAY_MS, MIN_SWAP_AMOUNT, sleep, validateSwapQuote, calculateFee, calculateSwapValueUsd, getSlippageBpsForSwap, calculateMinAmountOut, isPermanentFailure, } from './config';
import { classifyError, withRetry, decodeErrorSelector } from './error-handler';
 ...
 */
export function getTargetTokenAddress(targetAsset) {
 ...
 */
export function getTargetTokenDecimals(targetAsset) {
    if (targetAsset?.toLowerCase() === 'cbbtc') {
        return 8;
    }
    return 18;
 ...
 */
export function getTargetTokenSymbol(targetAsset) {
    if (targetAsset?.toLowerCase() === 'cbbtc') {
        return 'cbBTC';
    }
    return 'ETH';
...
let _cachedEthPriceUsd = null;
let _ethPriceCacheTimestamp = null;
...
export async function getETHPriceFromUniswap() {
    const now = Date.now();
...
// ============ SIMULATION ============
async function simulateSwap(userSmartAccount, tokenIn, amountIn, tokenOut) {
    try {
        ...
        const hasTokenOut = changes.some((c) => c.to?.toLowerCase() === userSmartAccount.toLowerCase());
        return { success: true };
    }
    catch (err) {
...
export async function runDryRunSimulation(delegations, decision) {
    console.log('\n========================================');
    ...
    for (const delegation of delegations) {
        const userSmartAccount = delegation.smart_account_address;
...
// ============ SWAP QUOTE ============
async function getSwapQuoteInternal(swapper, tokenIn, tokenOut, amount, slippageToleranceBps = SLIPPAGE_LARGE_BPS) {
    _quoteCounter++;
...
export async function getSwapQuote(swapper, tokenIn, tokenOut, amount, slippageToleranceBps) {
    const { result, error, attempts } = await withRetry(() => getSwapQuoteInternal(swapper, tokenIn, tokenOut, amount, slippageToleranceBps), { operation: 'getSwapQuote' });
    if (!result) {
...
async function executeDelegatedSwapViaUserOp(delegation, direction, swapTo, swapData, swapValue, nonceKey) {
...
export async function executeDelegatedSwapWithRetry(delegation, direction, swapTo, swapData, swapValue, nonceKey) {
    const { result, error, attempts } = await withRetry(() => executeDelegatedSwapViaUserOp(delegation, direction, swapTo, swapData, swapValue, nonceKey), { operation: 'executeDelegatedSwapViaUserOp', maxAttempts: 3, baseDelayMs: 2000 });
    if (!result) {
...
async function executeDelegatedSwapInternal(delegation, direction, swapTo, swapData, swapValue) {
    const signedDelegation = typeof delegation.delegation_data === 'string'
        ? JSON.parse(delegation.delegation_data)
        : delegation.delegation_data;
    if (!signedDelegation.signature) {
...
export async function executeDelegatedSwap(delegation, direction, swapTo, swapData, swapValue) {
    const { result, error, attempts } = await withRetry(() => executeDelegatedSwapInternal(delegation, direction, swapTo, swapData, swapValue), { operation: 'executeDelegatedSwap', maxAttempts: 3, baseDelayMs: 2000 });
    if (!result) {
        const errorMsg = error?.message || '';
        const selectorMatch = errorMsg.match(/0x[a-fA-F0-9]{8}/);
        const decodedError = selectorMatch ? decodeErrorSelector(selectorMatch[0]) : errorMsg;
...
export async function sendBatchedUserOps(batchItems, backendSmartAccount) {
...
export async function waitForBatchedUserOpReceipts(results) {
...
async function prepareSwap(walletData, decision, nonceKey, ethPriceUsd) {
    ...
    let swapQuote;
    try {
        swapQuote = await getSwapQuote(smartAccountAddress, tokenIn, tokenOut, swapAmountAfterFee.toString(), slippageBps);
    }
    catch (error) {
        const errorInfo = {
            stage: 'quote_fetch',
        ...
        walletData.__prepareError = errorInfo;
        return null;
    }
    if (!swapQuote) {
        const errorInfo = {
...
async function buildUserOpForSwap(backendSmartAccount, preparedSwap, decision) {
    ...
    try {
        ...
        if (!signedDelegation.signature) {
            ...
            walletData.__buildError = errorInfo;
            return null;
        }
        const execution = createExecution({
            target: swapQuote.swap.to,
            value: BigInt(swapQuote.swap.value || '0'),
            callData: swapQuote.swap.data,
        });
        const redeemCalldata = DelegationManager.encode.redeemDelegations({
            delegations: [[signedDelegation]],
            modes: [ExecutionMode.SingleDefault],
...
async function executeSwapWithUserOp(walletData, decision, nonceKey, ethPriceUsd) {
    const { delegation, smartAccountAddress, swapAmountAfterFee, fee } = walletData;
    const isBuy = decision.action === 'buy';
    // Get target token based on user's preference (cbBTC or WETH)
    const targetToken = getTargetTokenAddress(delegation.target_asset);
    ...
    let totalRetries = 0;
    const swapQuote = await getSwapQuote(smartAccountAddress, tokenIn, tokenOut, swapAmountAfterFee.toString(), slippageBps);
    if (!swapQuote) {
        return {
            success: false,
            txHash: null,
    ...
    const swapResult = await executeDelegatedSwapWithRetry(delegation, decision.action, swapQuote.swap.to, swapQuote.swap.data, BigInt(swapQuote.swap.value || '0'), nonceKey);
...
export async function processSwapsParallel(delegations, decision, fgValue) {
    const isBuy = decision.action === 'buy';
    ...
    await Promise.all(delegations.map(async (delegation) => {
        const smartAccountAddress = delegation.smart_account_address;
        try {
    ...
    for (let batchIndex = 0; batchIndex < walletDataList.length; batchIndex += OPTIMAL_BATCH_SIZE) {
        ...
        const preparedSwaps = await Promise.all(batch.map((walletData, index) => {
        ...
        const validPreparedSwaps = preparedSwaps.filter((s) => s !== null);
        ...
        if (validPreparedSwaps.length === 0) {
            for (const walletData of batch) {
                const prepareError = walletData.__prepareError;
        ...
        const batchItems = await Promise.all(validPreparedSwaps.map(preparedSwap => buildUserOpForSwap(backendSmartAccount, preparedSwap, decision)));
        const validBatchItems = batchItems.filter((b) => b !== null);
        ...
        if (validBatchItems.length === 0) {
            for (const preparedSwap of validPreparedSwaps) {
                const buildError = preparedSwap.walletData.__buildError;
        ...
        for (const walletData of batch) {
            if (!processedWallets.has(walletData.smartAccountAddress)) {
                ...
                const prepareError = walletData.__prepareError;
                const buildError = walletData.__buildError;
                let errorDetail;
...
export async function retrySwapWithOriginalAmounts(walletData, decision, fgValue) {
    const { delegation, smartAccountAddress, swapAmountAfterFee, fee } = walletData;
    const isBuy = decision.action === 'buy';
    // Get target token based on user's preference (cbBTC or WETH)
    const targetToken = getTargetTokenAddress(delegation.target_asset);
    ...
    const swapResult = await executeDelegatedSwap(delegation, decision.action, swapQuote.swap.to, swapQuote.swap.data, BigInt(swapQuote.swap.value || '0'));
    if (!swapResult.txHash) {
        return {
            success: false,
            txHash: null,
...
export async function processUserDCA(delegation, decision, fgValue) {
    const userAddress = delegation.user_address;
    const smartAccountAddress = delegation.smart_account_address;
    let totalRetries = 0;
    ...
    let balance;
    try {
        const { result: balanceResult, error: balanceError, attempts } = await withRetry(async () => isBuy
            ? await getUSDCBalance(smartAccountAddress)
            : await getETHBalance(smartAccountAddress), { operation: 'getBalance' });
