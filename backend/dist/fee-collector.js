...
import { ADDRESSES, emberStakingAbi } from './config';
import { publicClient, walletClient, backendAccount } from './clients';
async function executeDelegatedFeeTransfer(delegation, tokenAddress, amount) {
    if (amount === 0n)
        return null;
    try {
...
export async function collectFee(delegation, tokenAddress, amount) {
    if (amount === 0n)
        return null;
    try {
