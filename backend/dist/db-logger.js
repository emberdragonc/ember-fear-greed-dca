// ============ DATABASE LOGGING ============
import { DB_RETRY_CONFIG } from './config';
import { withRetry } from './error-handler';
import { supabase } from './clients';
export async function logExecution(delegationId, userAddress, fgValue, decision, result, isRetry = false) {
    const { error, attempts } = await withRetry(async () => {
...
export async function logFailedAttempt(delegationId, userAddress, stage, errorInfo, context) {
    ...
    const { error, attempts } = await withRetry(async () => {
        const { error } = await supabase.from('dca_failed_attempts').insert({
            ...
            retryable: errorInfo.retryable,
            context: JSON.stringify(context, (key, value) => typeof value === 'bigint' ? '0x' + value.toString(16) : value),
            created_at: new Date().toISOString(),
        });
        if (error)
            throw error;
...
export async function updateProtocolStats(volume, fees) {
    const { error, attempts } = await withRetry(async () => {
