...
import { ADDRESSES, SLIPPAGE_LARGE_BPS, OPTIMAL_BATCH_SIZE, BATCH_DELAY_MS, sleep, permit2Abi, } from './config';
import { publicClient, backendAccount, bundlerClient, pimlicoPaymasterClient, supabase, getETHBalance, getUSDCBalance, } from './clients';
...
import { executeDelegatedERC20ApprovalViaUserOp, executeDelegatedPermit2ApprovalViaUserOp, } from './approvals';
...
async function runRebalance() {
    ...
    for (const [wallet, target] of Object.entries(REBALANCE_TARGETS)) {
        ...
        const nativeEthBalance = await getETHBalance(wallet);
    ...
    for (let idx = 0; idx < items.length; idx++) {
        ...
        if (needsERC20) {
            ...
            const txHash = await executeDelegatedERC20ApprovalViaUserOp(item.delegation, ADDRESSES.WETH, ADDRESSES.PERMIT2, maxApproval, nonceKeyBase);
            if (txHash) {
        ...
        if (needsPermit2) {
            ...
            const txHash = await executeDelegatedPermit2ApprovalViaUserOp(item.delegation, ADDRESSES.WETH, ADDRESSES.UNISWAP_ROUTER, nonceKeyBase + 1n);
            if (txHash) {
    ...
    for (let i = 0; i < items.length; i += OPTIMAL_BATCH_SIZE) {
        ...
        for (let j = 0; j < batch.length; j++) {
            ...
            const swapQuote = await getSwapQuote(item.wallet, ADDRESSES.WETH, // tokenIn: WETH
            ADDRESSES.USDC, // tokenOut: USDC
            item.swapAmount.toString(), SLIPPAGE_LARGE_BPS);
            if (!swapQuote) {
