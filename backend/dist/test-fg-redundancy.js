...
function isFearGreedStale(data) {
...
function calculateBackupFG(btcChangePercent) {
    ...
    if (btcChangePercent <= BACKUP_ORACLE_THRESHOLDS.EXTREME_FEAR_DROP) {
        ...
        if (value <= 25)
            ...
            classification = 'Fear (Backup Oracle)';
        else if (value <= 54)
            classification = 'Neutral (Backup Oracle)';
        else if (value <= 75)
            classification = 'Greed (Backup Oracle)';
        else
            classification = 'Extreme Greed (Backup Oracle)';
    }
