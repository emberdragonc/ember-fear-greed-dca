...
import { ADDRESSES, permit2Abi, } from './config';
...
// ============ APPROVAL CHECKING ============
export async function checkUSDCApproval(smartAccountAddress) {
    try {
...
export async function checkPermit2Allowance(smartAccountAddress) {
    try {
...
export async function executeDelegatedERC20ApprovalViaUserOp(delegation, tokenAddress, spenderAddress, amount, nonceKey) {
    try {
...
export async function executeDelegatedPermit2ApprovalViaUserOp(delegation, tokenAddress, spenderAddress, nonceKey) {
    try {
        const backendSmartAccount = await initBackendSmartAccount();
        const signedDelegation = typeof delegation.delegation_data === 'string'
            ? JSON.parse(delegation.delegation_data)
...
export async function executeDelegatedERC20Approval(delegation, tokenAddress, spenderAddress, amount) {
    try {
        const signedDelegation = typeof delegation.delegation_data === 'string'
            ? JSON.parse(delegation.delegation_data)
            : delegation.delegation_data;
...
export async function executeDelegatedPermit2Approval(delegation, tokenAddress, spenderAddress) {
    try {
        const signedDelegation = typeof delegation.delegation_data === 'string'
            ? JSON.parse(delegation.delegation_data)
            : delegation.delegation_data;
...
export async function submitApprovalUserOps(task, tokenIn, nonceKeyBase) {
    const { delegation, smartAccountAddress, needsERC20, needsPermit2 } = task;
    try {
        let erc20TxHash = null;
        let permit2TxHash = null;
        if (needsERC20) {
            ...
            erc20TxHash = await executeDelegatedERC20ApprovalViaUserOp(delegation, tokenIn, ADDRESSES.PERMIT2, maxApproval, erc20NonceKey);
            if (!erc20TxHash) {
                return {
        ...
        if (needsPermit2) {
            ...
            permit2TxHash = await executeDelegatedPermit2ApprovalViaUserOp(delegation, tokenIn, ADDRESSES.UNISWAP_ROUTER, permit2NonceKey);
            if (!permit2TxHash) {
                return {
                    wallet: smartAccountAddress,
                    success: false,
...
export async function processApprovals(delegations, isBuy) {
    ...
    const needsApproval = [];
    const approvalChecks = await Promise.all(delegations.map(async (delegation) => {
    ...
    const approvalResults = await Promise.all(needsApproval.map((task, index) => {
