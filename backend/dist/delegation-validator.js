...
import { CAVEAT_ENFORCERS, DB_RETRY_CONFIG } from './config';
import { withRetry } from './error-handler';
import { supabase } from './clients';
export function validateDelegationCaveats(delegationData) {
...
export async function getActiveDelegations(targetWallet) {
    ...
    if (targetWallet) {
        const filtered = result.filter((d) => d.smart_account_address?.toLowerCase() === targetWallet ||
